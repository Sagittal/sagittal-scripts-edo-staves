...
const computeSubsectionsForEachFifth = (edo: Edo): Subsection[][] => {
    ...
    const subsections: Subsection[] =
        gatherSubsectionsForEdoNotationName(edoNotationName)

    const secondBestFifthEdoNotationName: EdoNotationName =
        `${edo}b` as EdoNotationName
    if (
        !isUndefined(EDO_NOTATION_DEFINITIONS[secondBestFifthEdoNotationName])
    ) {
        const secondBestFifthSubsections: Subsection[] =
            gatherSubsectionsForEdoNotationName(secondBestFifthEdoNotationName)
